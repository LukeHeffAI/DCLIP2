...
# ─── Build embeddings for baseline + methods ─────────────────────────────────────
all_embeddings  = []
all_group_names = []
all_class_names = []
marker_list     = []
...
for i, (grp, rng) in enumerate(zip(all_group_names, ranges)):
    ...
    if i == 0:
        # baseline: annotate every ANNOTATE_BASELINE_EVERY points
        for j in rng:
            if (j - rng.start) % ANNOTATE_BASELINE_EVERY == 0:
                ax.text(X_2d[j,0], X_2d[j,1],
                        flat_names[j],
                        fontsize=FONT_BASELINE,
                        color='black')
    else:
        # methods: annotate every ANNOTATE_METHOD_EVERY points
        for j in rng:
            if (j - rng.start) % ANNOTATE_METHOD_EVERY == 0:
                ax.text(X_2d[j,0], X_2d[j,1],
                        flat_names[j],
